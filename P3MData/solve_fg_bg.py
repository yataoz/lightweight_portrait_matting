...
# code copied from https://github.com/MarcoForte/closed-form-matting/blob/master/closed_form_matting/solve_foreground_background.py
...
def worker(img_file):
    ...
    mask_file = img_file.replace('blurred_image', 'mask').replace('.jpg', '.png')
    mask = cv2.imread(mask_file, cv2.IMREAD_GRAYSCALE) / 255.
    foreground, background = solve_foreground_background(img, mask)
    fg_file = img_file.replace('blurred_image', 'fg')
    bg_file = img_file.replace('blurred_image', 'bg')
    cv2.imwrite(fg_file, (foreground * 255).astype(np.uint8))
    cv2.imwrite(bg_file, (background * 255).astype(np.uint8))
    #print("fg min, max:", foreground.min(), foreground.max())
    #print("bg min, max:", background.min(), background.max())
    #cv2.imwrite('../tmp/fg.jpg', (foreground * 255).astype(np.uint8))
    #cv2.imwrite('../tmp/bg.jpg', (background * 255).astype(np.uint8))
    #cv2.imwrite('../tmp/comb.jpg', ((foreground * mask[..., np.newaxis] + background * (1 - mask[..., np.newaxis])) * 255).astype(np.uint8))
    #pdb.set_trace()

def main():
    num_processes = 32
    src_root = os.path.expanduser('./Datasets/P3M-10K/train')
    fg_dir = os.path.join(src_root, 'fg')
    bg_dir = os.path.join(src_root, 'bg')
    if not os.path.exists(fg_dir):
        os.makedirs(fg_dir)
    if not os.path.exists(bg_dir):
        os.makedirs(bg_dir)

    img_files = glob.glob(os.path.join(src_root, 'blurred_image/*.jpg'))
    with Pool(num_processes) as p:
        p.map(worker, img_files)
